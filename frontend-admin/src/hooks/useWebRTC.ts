...
export const useWebRTC = (socket: any, user: any, options?: UseWebRTCOptions) => {
    ...
    const initializeUserMedia = useCallback(async (enableVideo: boolean) => {
        ...
        try {
            ...
            // SỬA: Fallback sang voice-only và hiện toast cảnh báo
    ...
    const handleIncoming = useCallback(
        (data: any) => {
            console.log('[DEBUG-CALL] Received call:incoming', data)
            const callerId = normalizeUserId(data?.callerId)
            const callId = normalizeUserId(data?.callId)
            if (!callerId || !callId) {
                console.log('[DEBUG-CALL] Missing callerId or callId')
                return
            }
    ...
    useEffect(() => {
        ...
        const handleSocketDisconnect = () => {
            if (phaseRef.current !== 'IDLE') {
                // THÊM MỚI: Báo lỗi mất mạng nếu đang IN_CALL
                if (phaseRef.current === 'IN_CALL') {
                    toast.error('Mất kết nối — cuộc gọi đã kết thúc', { duration: 5000 })
                }
                resetCallState('ENDED')
            }
        }
